...
func GenerateKeyPairs() (*KeyPairs, error) {
	...
	kemKeyPair, err := GenerateKEMKeyPair(mlkem768.Scheme())
	if err != nil {
...
func OpenChannel(baseURLString string, env Env, profile Profile, httpClient *http.Client) (*Channel, error) {
	// validate base URL
	...
	m1.KemPublicData = keys.KEMKeyPair.PublicData

	// construct URL for starting the channel
	startChannelURL := baseURL.ResolveReference(&url.URL{Path: profile.ChannelPath})
	...
	var cid = firstHttpResponse.Header.Get(profile.HeaderNameCid)
	matched, err := regexp.MatchString("^/[A-Za-z0-9-/]{1,200}$", cid)
	if err != nil {
		return nil, fmt.Errorf("matching CID: %w", err)
	} else if !matched {
		return nil, fmt.Errorf("CID does not match regex")
	}
	// build the channel specific URL
	channelURL := baseURL.ResolveReference(&url.URL{Path: cid})

	slog.Debug("Received Message2", "cid", cid, "channelURL", channelURL.String())
	...
	if err := validateSignedPublicKeys(httpClient, baseURL, signedPubKeys); err != nil {
	...
	kem_ss, kem_ct, err := pubKeys.KemPublicData.Encapsulate()
	if err != nil {
...
func PostMessage[M interface{}](httpClient *http.Client, url string, requestMessage interface{}) ([]byte, *http.Response, []byte, *M, error) {
	...
	if err != nil {
		log.Fatalf("creating request to host: %v", err)
	}
	req.Header.Set("Content-Type", "application/cbor")

	resp, err := httpClient.Do(req)
	if err != nil {
		return nil, nil, nil, nil, fmt.Errorf("sending request to host: %w", err)
	}
...
func validateSignedPublicKeys(httpClient *http.Client, baseURL *url.URL, signedPubKeys *SignedPublicKeys) error {
