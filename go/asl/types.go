...
type PublicKeys struct {
	ECDH_PK       ECDHData
	KemPublicData KEMData `cbor:"ML-KEM-768_PK"`
	IssuedAt      int64   `cbor:"iat"`
	ExpiresAt     int64   `cbor:"exp"`
	Commment      string  `cbor:"comment"`
}

type SignedPublicKeys struct {
	SignedPubKeys    *PublicKeys `cbor:"-"`
...
func (c *CertData) MarshalCBOR() ([]byte, error) {
	rcaChain := make([][]byte, 0, len(c.RCAChain))
	for _, cert := range c.RCAChain {
		rcaChain = append(rcaChain, cert.Raw)
	}
	raw := certDataRaw{
		Cert:     c.Cert.Raw,
		CA:       c.CACert.Raw,
...
func (m *MessageError) Error() string {
	return fmt.Sprintf("asl: %d %s", m.ErrorCode, m.ErrorMessage)
}
