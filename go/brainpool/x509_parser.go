// Modified Version of the x509 package from the Go standard library.
...
func parseCertificatePoliciesExtension(der cryptobyte.String) ([]x509.OID, error) {
	...
	for !der.Empty() {
		...
		oidInts, ok := parseObjectIdentifier(OIDBytes)
		if !ok {
			return nil, errors.New("x509: invalid certificate policies")
		}
		oid, err := x509.OIDFromInts(oidInts)
		if err != nil {
			return nil, errors.New("x509: invalid certificate policies")
		}
		...
		if len(e.Id) == 4 && e.Id[0] == 2 && e.Id[1] == 5 && e.Id[2] == 29 {
			...
			case 32:
				...
				for _, oid := range out.Policies {
					if oid, ok := parseOIDString(oid.String()); ok {
...
// ----------------------------------------------------------------------------
// Copied internal functions from crypto/x509
// ----------------------------------------------------------------------------
