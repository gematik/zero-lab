...
func TestParseTSLDetachedSignature_HappyPath(t *testing.T) {
	...
	parsed, err := tsl.ParseSignature(sigBytes)
	require.NoError(t, err)
...
func TestParseTSLDetachedSignature_TamperedSignature(t *testing.T) {
	...
	parsed, err := tsl.ParseSignature(tampered)
	if err != nil {
...
func TestParseTSLDetachedSignature_RSAOIDRejected(t *testing.T) {
	...
	_, err = tsl.ParseSignature(sigBytes)
	require.Error(t, err)
	assert.True(t, errors.Is(err, tsl.ErrRSANotSupported),
...
func TestParseTSLDetachedSignature_UnknownAlgorithmRejected(t *testing.T) {
	...
	assert.False(t, errors.Is(err, tsl.ErrRSANotSupported))
}
...
func TestParseTSLDetachedSignature_TruncatedContainerRejected(t *testing.T) {
	...
	_, err := tsl.ParseSignature(sigBytes[:len(sigBytes)-5])
	require.Error(t, err)
}
...
func TestParseTSLDetachedSignature_WrongOuterTagRejected(t *testing.T) {
	...
	_, err := tsl.ParseSignature(bogus)
	require.Error(t, err)
...
func TestVerifyTSLDetachedSignature_HappyPath(t *testing.T) {
	...
	parsed, err := tsl.VerifySignature(
		t.Context(),
		sampleTSLBytes,
		sigBytes,
...
func TestLoadTSLDetachedSignature_HappyPath(t *testing.T) {
	...
	parsed, err := tsl.LoadSignature(t.Context(), srv.Client(), srv.URL+"/foo.sig")
	require.NoError(t, err)
	require.NoError(t, parsed.VerifyOver(sampleTSLBytes))
}
...
func TestLoadTSLDetachedSignature_Non200Rejected(t *testing.T) {
	...
	defer srv.Close()
	_, err := tsl.LoadSignature(t.Context(), srv.Client(), srv.URL+"/x.sig")
	require.Error(t, err)
...
func TestLoadTSLDetachedSignature_HonorsContextCancellation(t *testing.T) {
	...
	go func() {
		_, err := tsl.LoadSignature(ctx, srv.Client(), srv.URL+"/x.sig")
		done <- err
	}()
	...
	case <-time.After(1 * time.Second):
		t.Fatal("LoadSignature did not unblock client-side within 1s after cancel")
	}
}
...
func TestLoadTSLDetachedSignature_HonorsCustomHTTPClient(t *testing.T) {
	...
	parsed, err := tsl.LoadSignature(t.Context(), custom, "https://example.invalid/x.sig")
	require.NoError(t, err)
...
func TestTSLSignatureURL_ReplacesXmlWithSig(t *testing.T) {
	...
	for _, tc := range cases {
		assert.Equal(t, tc.want, tsl.SignatureURL(tc.in))
	}
}
