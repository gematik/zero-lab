...
// GET navigations (CSRF on state-changing endpoints is covered by the X-Requested-With check). Production
// adds the __Host- prefix + Secure (the strongest binding compatible with Lax); dev drops Secure so
// http://127.0.0.1 works. The cookie value is always the opaque session id; tokens never leave the server.
func newCookieTemplate(name string, production bool) *http.Cookie {
	c := &http.Cookie{
		Name:     name,
		Path:     "/",
		HttpOnly: true,
