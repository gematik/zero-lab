...
func newConnectorUseCmd() *cobra.Command {
	return &cobra.Command{
		...
		Long: "Set the active connector configuration. Subsequent commands without\n" +
			"-c/--connector-config or " + connectorConfigEnv + " env var will use this selection.\n\n" +
		...
		ValidArgsFunction: func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			...
			return completeConnectorConfigNames(cmd, args, toComplete)
		},
		RunE: func(cmd *cobra.Command, args []string) error {
			cmd.SilenceUsage = true
			name := args[0]
			if _, err := resolveConnectorConfigFile(name); err != nil {
				return err
			}
			path := activeConnectorFile()
