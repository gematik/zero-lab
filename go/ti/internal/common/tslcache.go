...
// 5-minute cache. On stale entries we run `IsUpdateAvailable` so the 4MB
...
// we degrade to a direct Load so the CLI keeps working.
func LoadTSLCached(ctx context.Context, httpClient *http.Client, url string) (*tsl.List, error) {
	if NoCache {
		return tsl.Load(ctx, httpClient, url)
	}
	...
	if hit && time.Since(entry.CachedAt) < tslCacheTTL {
		...
		return tsl.Parse(bytes.NewReader(entry.Raw), url)
	}

	if hit {
		updated, err := tsl.IsUpdateAvailable(ctx, httpClient, url, entry.Hash)
		if err == nil && !updated {
	...
	list, err := tsl.Load(ctx, httpClient, url)
	if err != nil {
		return nil, err
	}
