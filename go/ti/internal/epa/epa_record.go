...
func lookupRecord(ctx context.Context, st state.Store, env epa.Env, kvnr string) (*recordResult, error) {
	...
	// Cache hit?
	cached, hit, err := common.GetJSON[providerMappingEntry](st, kvnrProviderKey(env, kvnr))
	if err != nil {
		return nil, err
	}
	if hit {
		...
		for _, pr := range probes {
			if pr.Found {
				...
				if err := common.SetJSON(st, kvnrProviderKey(env, kvnr), entry, state.Expire(kvnrProviderTTL)); err != nil {
...
func renderRecord(r *recordResult) {
	if r.Provider == nil {
		...
		if len(r.Probes) > 0 {
			common.PrintTable("PROVIDER\tFOUND\tNOTE", func(w io.Writer) {
	...
	common.PrintTable("FUNCTION\tDECISION", func(w io.Writer) {
