...
func runPKCS12Inspect(file string) error {
	...
	for i, cb := range bags.Certificates {
		...
		fmt.Println(common.SectionHeader(fmt.Sprintf("--- Certificate %d ---", i+1)))
		common.PrintKeyValue(func(w io.Writer) {
			if cb.FriendlyName != "" {
			...
			if cert.PublicKey != nil {
				fmt.Fprintf(w, "Key Size\t%s\n", common.DescribePublicKey(cert.PublicKey))
			}
	...
	for i, kb := range bags.PrivateKeys {
		fmt.Println(common.SectionHeader(fmt.Sprintf("--- Private Key %d ---", i+1)))
		common.PrintKeyValue(func(w io.Writer) {
			if kb.FriendlyName != "" {
				fmt.Fprintf(w, "Friendly Name\t%s\n", kb.FriendlyName)
			}
...
func describePrivateKeyBag(raw []byte) string {
	...
	if key, err := brainpool.ParseECPrivateKey(raw); err == nil {
		return fmt.Sprintf("EC %s", common.DescribeECCurve(key.Curve))
	}
