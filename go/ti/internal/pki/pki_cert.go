...
func newPKICertCmd(def common.EnvDef) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "cert",
...
func newPKICertInspectCmd(def common.EnvDef) *cobra.Command {
...
func newPKICertVerifyCmd(def common.EnvDef) *cobra.Command {
...
func renderVerifyResultText(result *gempki.ValidationResult, opts certVerifyOpts) error {
	...
	if len(result.Chain) > 0 && result.Chain[0] != nil {
		...
		kv.KV("Serial Number", common.ColonHex(ee.SerialNumber.Bytes()))
		kv.KV("Not Before", ee.NotBefore.Format(time.RFC3339))
		kv.KV("Not After", ee.NotAfter.Format(time.RFC3339))
		kv.KV("Public Key", fmt.Sprintf("%s %s", ee.PublicKeyAlgorithm, common.DescribePublicKey(ee.PublicKey)))
		kv.EndSection()
	}
...
func verifyResultJSON(r *gempki.ValidationResult, opts certVerifyOpts) map[string]any {
	...
	if len(r.Chain) > 0 && r.Chain[0] != nil {
		...
		cert := map[string]any{
			...
			"serialNumber": common.ColonHex(ee.SerialNumber.Bytes()),
			"notBefore":    ee.NotBefore.Format(time.RFC3339),
			"notAfter":     ee.NotAfter.Format(time.RFC3339),
			"publicKey":    fmt.Sprintf("%s %s", ee.PublicKeyAlgorithm, common.DescribePublicKey(ee.PublicKey)),
		}
		if opts.detectedType != gempki.CertTypeUnknown {
