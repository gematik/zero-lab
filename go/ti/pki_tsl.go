...
func newPKITSLCmdGroup(def envDef) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "tsl",
...
func newPKITSLShowCmd(def envDef) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			tsl, err := loadTSLCached(cmd.Context(), newHTTPClient(), def.TSLURL)
			if err != nil {
				return err
			}
...
func runTSLShow(tsl *gempki.TrustServiceStatusList, url string, f outputFormat) error {
	if f == formatJSON {
		return printJSON(tslSummaryJSON(tsl, url))
	}
...
func newPKITSLFetchCmd(def envDef) *cobra.Command {
...
func runTSLFetch(ctx context.Context, def envDef, f outputFormat, outPath string, withSig bool) error {
	...
	if withSig {
		...
		sig, err := gempki.LoadTSLDetachedSignature(ctx, newHTTPClient(), sigURL)
		if err != nil {
...
func newPKITSLVerifyCmd(def envDef) *cobra.Command {
...
func runTSLVerify(ctx context.Context, def envDef, f outputFormat, sigPath string, at *time.Time) error {
...
func newPKITSLProvidersCmd(def envDef) *cobra.Command {
...
func runTSLProviders(tsl *gempki.TrustServiceStatusList, f outputFormat, sti string) error {
	...
	return printTable("PROVIDER\tSERVICE\tTYPE\tSTATUS\tCERT", func(w io.Writer) {
		for _, r := range rows {
...
func newPKITSLIntermediatesCmd(def envDef) *cobra.Command {
	var formatRaw string
	cmd := &cobra.Command{
		Use:   "intermediates",
...
func runTSLIntermediates(tsl *gempki.TrustServiceStatusList, f outputFormat) error {
	...
	return printTable("CN\tSTATUS\tNOT AFTER", func(w io.Writer) {
		for _, c := range cas {
			fmt.Fprintf(w, "%s\t%s\t%s\n",
